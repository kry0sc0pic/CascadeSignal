"""Tests for the T2 state-reconstruction correctness gate (CAS-28).
...
def test_reconstruct_hf_within_tolerance_band_is_not_mismatch():
    # CAS-28 ADR-005: HF_TOLERANCE=0.01 -- a reconstructed HF of 1.005 is
...
def test_reconstruct_hf_outside_tolerance_band_is_mismatch():
    # CAS-28 ADR-005: HF=1.02 clears 1.0 by more than HF_TOLERANCE=0.01, so
...
# ---------------------------------------------------------------------------
# Same-block-earlier-activity, generalized beyond cascades (CAS-28 H4-general)
# ---------------------------------------------------------------------------
...
def test_bucket_causes_ignores_dust_reserve_missing_coverage():
    ...
    # `unexplained`, not `unexplained_no_chainlink_coverage` -- CAS-28's
...
# ---------------------------------------------------------------------------
# Real-data T2 gate (CAS-28's actual enforcement mechanism)
# ---------------------------------------------------------------------------
...
def test_t2_gate_enforces_two_percent_mismatch():
    """The actual CAS-28 enforcement mechanism, now PASSING under ADR-005's
    ...
    Live-oracle fallback (CAS-28, Lever 12): applies
    ...
    liquidations = events[events["event_type"] == "LiquidationCall"]
    engine = PositionStateEngine(events=events, reserve_table=RESERVES)
    # CAS-28 H1: native per-position ETH-numeraire price, falling back to
    ...
    # CAS-28 Lever 3: point-in-time liquidation thresholds.
    try:
    ...
    # CAS-28 Lever 9 fix: pass the SAME oracle used for the primary
    ...
    # CAS-28 Lever 12: see docstring above.
