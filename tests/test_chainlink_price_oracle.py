"""Unit tests for `ChainlinkPriceOracle`, `BlendedPriceOracle`, and `EthNumeraire`/`PreferEthNumeraireOracle`."""
...
def test_eth_numeraire_default_has_no_staleness_limit(
 ...
 #: Aave's on-chain latestAnswer never checks staleness, so
...
def test_eth_numeraire_log_index_ordering_sees_earlier_same_block_update(tmp_path):
 # Same mechanism as ChainlinkPriceOracle, separate
...
# ---------------------------------------------------------------------------
# aggregator_eras clipping (/12, 2026-07-22)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# coverage_end (2026-07-22): a reserve-level cutoff for when
...
class _RecordingStubOracle(_StubOracle):
 ...
 through, not silently dropped by an intermediate wrapper."""
