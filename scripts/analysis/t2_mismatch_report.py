"""Full T2 mismatch diagnostics report (CAS-28).
...
`--live-oracle-fallback` (CAS-28) additionally re-checks every `unexplained`
...
def main() -> None:
    ...
    # Primary oracle (CAS-28 H1): native asset/ETH pricing per position where
    ...
    # Point-in-time liquidation thresholds (CAS-28 Lever 3): use the value in
    ...
    # CAS-28 Lever 9 fix: bucket against `price_oracle` itself, not a
