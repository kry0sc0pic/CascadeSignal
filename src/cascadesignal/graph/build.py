"""Contagion-graph snapshot builder (CAS-31).
...
(`state.engine.PositionStateEngine`, CAS-13/47), Aave's reserve registry,
and DEX pool depth (`graph.pool_depth`, CAS-16), applying its own whale/
...
T2 (CAS-28) passed via ADR-005 (tolerance-band mismatch rate 1.093% at
epsilon=1%, against the 2% gate). `scripts/graph/materialize_snapshots.py`
re-checks the live T2 rate and runs the full CAS-31 acceptance-criterion
production job (`build_snapshots` over the whole Aave v2 study period) --
