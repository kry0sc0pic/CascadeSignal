"""Event-based alert evaluation: recall @ fixed false-alarm budget,
lead-time frontier.

These operate on a continuous risk-score stream (one score per evaluation
block/bar) and cascade episodes ( `data/curated/labels/*/episodes.parquet`
schema: `start_block`, `end_block`). Definitions, since the spec leaves the
exact accounting open (: "Define 'protectable USD' accounting
