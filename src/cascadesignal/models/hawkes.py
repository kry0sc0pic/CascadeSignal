"""M1: Hawkes processes + branching ratio n(t) (CAS-18, Mock 1 slice).

Mock 1 scope note: the board's full M1 ticket (marked univariate ->
multivariate by collateral asset -> multiplex-network Hawkes) is split across
milestones -- CAS-18's own acceptance criteria call for univariate +
multivariate fits by Mock 1, with the multiplex-network (channel-specific
kernels over the contagion graph) staying Mock 2, since that needs the
contagion graph (CAS-13/graph, not built yet). This module holds both Mock 1
pieces:

* `HawkesUnivariateBranchingRatio` -- one self-exciting process on the
  aggregate liquidation-count series (CAS-3/E3, already validated: n(t) rises
...
* `HawkesMultivariateBranchingRatio` -- a mutually-exciting process over a
  ...
  composability) remain the CAS-18 Mock 2 follow-on once the graph exists.

Model (univariate): exponential-kernel Hawkes on the bar-level liquidation-
count series (marks = n_liquidations per 5-block bar from CAS-20).
...
convention), so it never depends on the bar's own count -- consistent with the
CAS-20 as-of contract. (mu, alpha, beta) are fit by Poisson MLE on the train
