"""Per-bar forecast labels for the walk-forward harness.

Turns the *episodes* (spans) into a per-bar binary *forecast*
target aligned to the: a bar is positive iff a cascade
 ...
 *,
 ...
 Args:
 bars: feature bars with an `end_block` column.
 episodes: cascade episodes with `start_block` (+ `is_primary`).
 ...
 *,
 ...
 indicator, for training a severity-forecast head alongside
