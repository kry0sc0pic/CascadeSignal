"""Aave v2 interest-index lookups (CAS-47 follow-up).
...
`scripts/onchain/backfill_reserve_index.py` (CAS-28 Track D), an Etherscan-
...
existed or outside its block range.

CAS-28 Track E wires this into `engine.PositionStateEngine`'s balance replay
...
(~1.0-and-growing) float.

CAS-28 H5 (2026-07-22): `index_at`/`index_at_many` return the index as of
...
class InterestIndexOracle:
    ...
    def index_at(
        ...
        `query_timestamp` (CAS-28 H5): if given, forward-compounds the
    ...
    def index_at_many(
        ...
        (Track E, CAS-28). Returns a float Series aligned to `reserves`'
        original index, with NaN wherever `index_at` would return None.

        `query_timestamps` (CAS-28 H5): same forward-compounding as
