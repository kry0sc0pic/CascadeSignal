"""T2 state-reconstruction correctness gate (CAS-28):
...
Tolerance band (CAS-28 ADR-005, 2026-07-23): sixteen levers drove the
...
Trigger-block convention (CAS-28 Lever 1, 2026-07-21): HF is computed from
...
being dropped as "no debt"). The earlier CAS-17
...
Same-block liquidation cascades (CAS-28 Lever 7/H4a, 2026-07-22): the
...
Same-block-earlier-activity, generalized (CAS-28 H4-general, 2026-07-22):
...
# CAS-28 ADR-005: a reconstructed HF only counts as a mismatch once it clears
...
class PriceOracleLike(Protocol):
    ...
    `block_number`/`log_index` (CAS-28 H4b): optional -- callers here always
...
def reconstruct_hf_at_trigger(
    ...
    If `config_history` is given (Lever 3, CAS-28), each trigger's HF uses the
    ...
    `compound_interest_to_trigger` (Lever 8, H5, CAS-28, default True):
    ...
    # Same-block-earlier-activity triggers (CAS-28 H4-general, generalizing
    ...
    # `after_log_index` (CAS-28 H4-general bugfix, see
    ...
    for key, position in position_by_key.items():
        ...
        # CAS-28 H4b: resolve strictly before this trigger's own log
        ...
        # CAS-28 ADR-005: tolerance-band mismatch condition. `mismatch_summary`
...
def mismatch_summary(report: pd.DataFrame) -> pd.DataFrame:
    ...
    CAS-28 ADR-005: `mismatch_rate` is the tolerance-band rate (`report`'s own
...
def bucket_mismatch_causes(
    ...
    `mismatch` column; see the CAS-28 ADR-005 note below) (index-aligned;
    non-mismatch rows get `None`):

    CAS-28 ADR-005 scoping note: this buckets (and `apply_live_oracle_fallback`
    ...
    - `param_drift`: the position touches a reserve whose current on-chain
      ...
      historical collateral value. With `config_history` (Lever 3, CAS-28)
    ...
    - `oracle_lag`: swapping in `chainlink_oracle`'s prices (where fully
      ...
      *primary* reconstruction used (originally DefiLlama-daily vs.
      Chainlink-block, CAS-17). CAS-28 Lever 9 fix (2026-07-22): callers
    ...
    - `unexplained_no_chainlink_coverage`: `chainlink_oracle` has no price for
      some reserve that actually matters to this position's HF (CAS-28,
      post-Lever-11c fix: only reserves with `collateral_units > _EPS` and
...
def apply_live_oracle_fallback(
    ...
    """Re-check every `unexplained` mismatch (CAS-28) against Aave v2's real,
    ...
    *reconstruction*, not in Aave's real liquidation decision.
    ...
    offline: a full-population spot-check (CAS-28, n=2,896 -- the entire
