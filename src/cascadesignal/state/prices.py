"""Price lookups for state reconstruction.
...
- `ChainlinkPriceOracle`: Chainlink `AnswerUpdated` events
...
- `BlendedPriceOracle`: Chainlink block-level price per
...
- `EthNumeraire`: prices every reserve natively in ETH via its
...
- `PreferEthNumeraireOracle`: per-position wrapper -- uses
...
class PriceOracle:
 ...
 `block_number`/`log_index`: accepted for interface
 ...
 every other correction's no-op-when-inapplicable convention.
 """
 if not address:
 return None
...
#: combined sortable (block_number, log_index) order key, used to
...
def _merge_aggregator_series(
 ...
 Sorted/deduped by `(block_number, log_index)`, not
 ...
 `aggregator_eras` (/12, 2026-07-22): optional per-address
...
class ChainlinkPriceOracle:
 ...
 events, restricted to the 5 golden-episode pull windows.
 ...
 HF-reconstruction query point H4b's log_index ordering is
 ...
 log_index)` if both are given, else at-or-before
 ...
 *after* the query's own log position is (correctly) invisible under
 ...
 """Vectorized `price_at` over many timestamps for one reserve.
...
class BlendedPriceOracle:
 ...
 (within its staleness window), DefiLlama daily otherwise.
...
class EthNumeraire:
 ...
 reserve's own Chainlink asset/ETH feed.
 ...
 but **no staleness guard by default**: the on-chain
 ...
 `block_number`/`log_index`: resolve strictly before
 ...
 `coverage_end`: some reserves' feed map entry
...
# keccak256("getAssetPrice(address)"), derived via Crypto.Hash.keccak
...
class LiveAaveOracleFallback:
 """Live on-chain price via Aave v2's real `AaveOracle.getAssetPrice(asset)`, queried by `eth_call` at the exact historical block on an
 ...
 discovered. Spot-checked against the "no coverage" residual (post-Lever-11c): `getAssetPrice(BUSD)` returns a real, economically sane
 ...
 check (n=2,896, not a sample) measured a 58.6% correction rate
 ...
 # "execution reverted" phrase (: an earlier, looser
